...
export interface CartItem {
  ...
  buyingPrice: number | null   // company buying price — visible to staff only
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const addItem = useCallback((item: Omit<CartItem, 'quantity' | 'customPrice'>) => {
    setItems((prev) => {
      ...
      return [...prev, { ...item, quantity: 1, customPrice: null, dealerPrice: item.dealerPrice ?? null, buyingPrice: item.buyingPrice ?? null }]
    })
  }, [])
